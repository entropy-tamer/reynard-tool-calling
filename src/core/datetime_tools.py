"""Date/Time tools for the Reynard assistant.
...
async def format_time_tool(
    ...
    try:
        ...
        if input_format == "auto":
            ...
            msg = f"Invalid input format: {input_format}"
            raise ValueError(msg)

        if not parsed_time:
            msg = f"Could not parse time input: {time_input}"
            raise ValueError(msg)
        ...
        if (
            ...
            if converted_time:
                ...
                msg = f"Could not convert to timezone: {target_timezone}"
                raise ValueError(msg)
...
class GetCurrentTimeTool(BaseTool):
    ...
    async def execute(self, context: ToolExecutionContext, **params) -> ToolResult:
        ...
        try:
            ...
            logger.exception(f"Error executing get_current_time tool: {e}")
...
class FormatTimeTool(BaseTool):
    ...
    async def execute(self, context: ToolExecutionContext, **params) -> ToolResult:
        ...
        try:
            ...
            logger.exception(f"Error executing format_time tool: {e}")
            return ToolResult(success=False, data={}, error=str(e))
