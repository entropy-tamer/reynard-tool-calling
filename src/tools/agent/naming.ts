...
export class NamingTools {
  static async generateAgentName(args: { specialist: string; style: string }): Promise<ToolResult> {
    try {
  ...
  static async assignAgentName(args: { agentId: string; name: string }): Promise<ToolResult> {
    try {
  ...
  static async getAgentName(args: { agentId: string }): Promise<ToolResult> {
    try {
      const { agentId } = args;
